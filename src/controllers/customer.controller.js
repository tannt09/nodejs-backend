...
class CustomerController {
  async register(req, res) {
    ...
    try {
      ...
      const usernameCheck = await client.query("SELECT 1 FROM customers WHERE username = $1", [username]);
  ...
  async login(req, res) {
    ...
    try {
      ...
      const userCheck = await client.query("SELECT * FROM customers WHERE username = $1", [username]);
